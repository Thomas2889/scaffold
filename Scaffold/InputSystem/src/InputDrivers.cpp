...
namespace powd::input
{
	...
	template<> glm::vec3 InputDriver::ReadValue()
	{
		if (GetType() != Type::_vec3)
		{
			cpplog::Logger::Log("Attempted to read type Vec3 on an InputDriver of type: " + TypeToStr(GetType()), cpplog::Logger::WARNING);
			return { 0, 0, 0 };
		}
		return *(glm::vec3*)ReadValue_Internal();
	}
	...
	template<> void InputDriver::SetValue(glm::vec3 val)
	{
		if (GetType() != Type::_vec3)
			cpplog::Logger::Log("Attempted to set type Vec3 on an InputDriver of type: " + TypeToStr(GetType()), cpplog::Logger::WARNING);
		else
			SetValue_Internal(&val);
	}
	...
	void InputDriverVec3::SetValue_Internal(void* val)
	{
		myval = *(glm::vec3*)val;
		eventSignal.publish(this);
		children["x"]->SetValue<float>(1.f);
		children["y"]->SetValue<float>(1.f);
		children["z"]->SetValue<float>(1.f);
	}
}
