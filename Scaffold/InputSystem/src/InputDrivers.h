...
namespace powd::input
{
	...
	class InputDriverVec3 : public InputDriver
	{
	public:
		glm::vec3 myval;
		InputDriverVec3(std::string _name, glm::vec3 _val, InputDevice* _device, InputDriver* _parent = nullptr) : myval(_val), InputDriver(_name, _device, _parent)
		{
			new InputDriverFloat("x", 0, _device, this);
			new InputDriverFloat("y", 0, _device, this);
			new InputDriverFloat("z", 0, _device, this);
		}

	private:
		void* ReadValue_Internal() { return &myval; };
		void SetValue_Internal(void* val);
		Type GetType() { return Type::_vec3; }
	};
}
