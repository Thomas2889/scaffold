...
namespace scaffold::input
{
	...
	template<> glm::vec3 InputAction::GetData()
	{
		return data._vec3;
	}
	...
	void InputAction::AddBinding(std::string binding)
	{
		...
		case InputDriver::Type::_vec2:
			...
			break;
		case InputDriver::Type::_vec3:
			tmpData._vec3 = leadDriver->ReadValue<glm::vec3>();
			break;
		}
		ProcessData(tmpData);
	}
	...
	void InputAction::OnDataChange(InputDriver* driver)
	{
		...
		case InputDriver::Type::_vec2:
			...
			break;
		case InputDriver::Type::_vec3:
			tmpData._vec3 = driver->ReadValue<glm::vec3>();
			if (leadDriver == driver)
			{
				PickNewLeader();
			}
			else if (glm::length2(tmpData._vec3) > glm::length2(leadDriver->ReadValue<glm::vec3>()))
			{
				leadDriver = driver;
				ProcessData(tmpData);
			}
	...
	void InputAction::PickNewLeader(InputDriver* ignore)
	{
		...
		for (const auto& driverPair : drivers)
		{
			...
			case InputDriver::Type::_float:
				...
				break;
			case InputDriver::Type::_vec2:
				if (glm::length2(driverPair.second->ReadValue<glm::vec2>()) >= glm::length2(tmpData._vec2))
				{
					tmpData._vec2 = driverPair.second->ReadValue<glm::vec2>();
					tmpLead = driverPair.second;
				}
				break;
			case InputDriver::Type::_vec3:
				if (glm::length2(driverPair.second->ReadValue<glm::vec3>()) >= glm::length2(tmpData._vec3))
				{
					tmpData._vec3 = driverPair.second->ReadValue<glm::vec3>();
					tmpLead = driverPair.second;
				}
				break;
			}
